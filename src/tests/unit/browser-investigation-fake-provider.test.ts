...
import {
  ...
  type StoredInvestigationInsert,
...
function createMemoryStore(): InvestigationStore & { rows: Map<string, InvestigationRecord> } {
  ...
  return {
    ...
    findActive: (ws, domain) =>
      [...rows.values()].find((r) => r.workspaceId === ws && r.domain === domain && (r.status === 'queued' || r.status === 'running')) ??
      null,
