...
- **ProductOnPages:** assign only pages from the store's defined page list (211 pages in the ShopSite pages table) — page names are **not free text**. Every assigned name must match a real page `Name` exactly; never invent one. The always-available authority is a pages download (`db_xml.cgi`, `dbname=pages`); the redacted fixture shows the shape. Emit uses the modern `<ProductOnPages><PageLink><Name>` layout (see `shopsite-product-fields`). Observed category pages (examples, never exhaustive): Dog Healthcare, Pest Control, Pest Control & Animal Repellents, Jerky Dog Treats, Wood Pellets, Plants, Brand pages (e.g. Brand - Science Diet), Special Offers.
